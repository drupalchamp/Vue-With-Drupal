...
function dataUnpack(message) {
  ...
  currentPrice[pair]['CHANGE24HOURPCT'] =
    currentPrice[pair]['CHANGE24HOURPCT'] == 'NaN%'
      ? '0.00%'
      : currentPrice[pair]['CHANGE24HOURPCT'];
...
function decorateWithFullVolume(message) {
  ...
  currentPrice[pair]['FULLVOLUMETO'] =
    (currentPrice[pair]['FULLVOLUMEFROM'] -
      currentPrice[pair]['VOLUME24HOUR']) *
    ...
    if (messageToDisplay[key]) {
      if (fields[key].Show) {
        switch (fields[key].Filter) {
          case 'String':
            jQuery('[id="' + key + '_' + from + '"]').text(
              messageToDisplay[key]
            );
            break;
          ...
          case 'Number':
            ...
            if (key == 'VOLUME24HOURTO') {
              var OLD_RAW_VOLUME24HOURTO = jQuery(
                '#VOLUME24HOURTO_' + from
              ).attr('data-raw');
              var NEW_RAW_VOLUME24HOURTO = messageToDisplay[key];

              OLD_RAW_VOLUME24HOURTO =
                OLD_RAW_VOLUME24HOURTO !== undefined
                  ? OLD_RAW_VOLUME24HOURTO
                  : 0;
              ...
              jQuery('[id="' + key + '_' + from + '"]').text(
                CCC.convertValueToDisplay(symbol, messageToDisplay[key])
              );
              jQuery('[id="' + key + '_' + from + '"]').attr(
                'data-raw',
                messageToDisplay[key]
              );
            } else {
